...
#include <string.h>
#include "tests.h"
#define CERT_MAX_SIZE 2000
static char _CERT[CERT_MAX_SIZE];
size_t read_file(char* name)
{
    size_t newLen = 0;
    FILE* fp = fopen(name, "r");
    if (fp != NULL)
    {
        newLen = fread(_CERT, sizeof(char), 2000, fp);
        ...
        {
            _CERT[newLen++] = '\0'; /* Just to be safe. */
        }

        fclose(fp);
    }
    printf("Length of the certificate is %lu \n", newLen);
    return newLen;
}
...
static void _test_asn1_parsing(void)
{
    ...
    printf("=== begin %s()\n", __FUNCTION__);
    certificate_length = read_file("../data/asn1_cert.pem");
    OE_TEST(oe_cert_read_pem(&cert, _CERT, certificate_length) == OE_OK);
